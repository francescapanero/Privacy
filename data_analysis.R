...
# QUESTO E' QUELLO CHE FUNZIONA MEGLIO
...
# table_nospace <- prova2sub %>% group_by(STATEICP, HHINCOME, VALUEH, FAMSIZE, NCHILD, SEX, AGE, MARST, 
...
# 
# 
# tablestate <- prova2sub %>% group_by(EDUC, SCHLTYPE, EMPSTAT, OCC, 
#                                      INCTOT, FTOTINC, VETSTAT) %>% count()
# freq_state <- sort(tablestate$n)
# 
# m_pl = displ$new(freq_state)
...
# id_state <- prova2sub %>% add_count(EDUC, SCHLTYPE, EMPSTAT, OCC, 
#                                     INCTOT, FTOTINC, VETSTAT) %>% filter(n==1) %>% select(id)
# id_state <- id_state$id
...
estimates <- loop_estimate(data=prova2sub, id_uniq_pop=id_state, percentage=0.01, iter=2, 
