...
training_test_PY <- function(frequencies, percentage = 0.75, step = 100) {
  ...
  p <- ggplot(data_plot, aes(x = nn, y = est_tau1)) +
    theme_bw() +
    xlab("# of observations") +
    ylab(expression(tau[1])) +
    geom_point(aes(y = true_tau1)) +
    geom_line(col = "gold")
  p
}
training_test_DP <- function(frequencies, percentage = 0.75, step = 100) {
  ...
  p <- ggplot(data_plot, aes(x = nn, y = est_tau1, ymin = lower_tau1, ymax = upper_tau1)) +
    theme_bw() +
    xlab("# of observations") +
    ylab(expression(tau[1])) +
    geom_point(aes(y = true_tau1)) +
